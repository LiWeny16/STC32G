...
void calculate_s(DG_State *dg, Err_Steering *Err_Ste) // 根据电感状态计算偏差值，并更新偏差值结构体中last量，结果保存在舵机偏差值结构体中
{
  ...
  dg->L_zh_real = (int)(ADC_ReadAverage(ZH, count_dg, RE)); // 最大值255

	dg->L_zs_real = (int)(ADC_ReadAverage(ZS, count_dg, RE));//
	dg->L_ys_real = (int)(ADC_ReadAverage(YS, count_dg, RE));//

  dg->L_yh_real = (int)(ADC_ReadAverage(YH, count_dg, RE)); // 最大值255
  ...
  // dg->L_zx_once = (sqrt((()dg->L_zx_real) / (dg->L_zx_max)));
  // dg->L_zh_once = (sqrt((()dg->L_zh_real) / (dg->L_zh_max)));
  // dg->L_zs_once = (sqrt((()dg->L_zs_real) / (dg->L_zs_max)));//没用
  // dg->L_ys_once = (sqrt((()dg->L_ys_real) / (dg->L_ys_max)));//没用
  // dg->L_yh_once = (sqrt((()dg->L_yh_real) / (dg->L_yh_max)));
  // dg->L_yx_once = (sqrt((()dg->L_yx_real) / (dg->L_yx_max)));
  dg->L_zx_once = (((float)dg->L_zx_real) / (dg->L_zx_max));
  dg->L_zh_once = (((float)dg->L_zh_real) / (dg->L_zh_max));
	
  dg->L_yh_once = (((float)dg->L_yh_real) / (dg->L_yh_max));
  dg->L_yx_once = (((float)dg->L_yx_real) / (dg->L_yx_max));
	
	dg->L_zs_once = (((float)dg->L_zs_real) / (dg->L_zs_max));
  dg->L_ys_once = (((float)dg->L_ys_real) / (dg->L_ys_max));
